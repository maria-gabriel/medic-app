...
import es.dmoral.toasty.Toasty;
    ...
    //Ventana para configuración de cuenta de médico
public class Config1 extends AppCompatActivity implements Response.Listener<JSONObject>, Response.ErrorListener{
    RequestQueue rq;
    JsonRequest jrq;
    String Correo, LoadTel,LoadCon;
    EditText cor,tel,con,con2,edit;
    Button b1,b2,b3;
    TextView text;
    ...
    //Función en caso de que la petición para obtener los datos del médico no se completo correctamente
    public void onErrorResponse(VolleyError error) {
        Toast.makeText(getApplicationContext(),"Error al cargar datos",Toast.LENGTH_SHORT).show();
    }

    //Función en caso de que la petición para obtener los datos del médico se completo correctamente
    ...
    public void Guardar(View view) {
        if(cor.getText().length()==0 || tel.getText().length()==0 || con.getText().length()==0 || con.getText().length()==0){
            ...
            if(Resultado.find()==true){
                if(con.getText().toString().equals(con2.getText().toString())){
                    new Config1.Enviar(Config1.this).execute();
                }else{
    ...
    public boolean Insertar(){
        ...
        String IP = getString(R.string.ip);
        httpPost = new HttpPost("http://"+IP+"/android/modificar5.php?Correo="+Correo);
