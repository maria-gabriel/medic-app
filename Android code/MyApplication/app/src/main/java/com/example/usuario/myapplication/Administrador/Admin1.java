...
import java.util.regex.Pattern;
    ...
    //Ventana para gestión de pacientes
public class Admin1 extends AppCompatActivity implements Response.Listener<JSONObject>, Response.ErrorListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        String[] Campo = {"Campo","Nombre","ApellidoP","ApellidoM","Correo","Edad","Telefono","Contra"};
    ...
    public void Registrar(View view) {
        ...
        if(name.getText().length()==0 || name2.getText().length()==0 || name3.getText().length()==0 ||  email.getText().length()==0 || edad.getText().length()==0 ||
            ...
            //Plantilla para validar el correo electrónico
            ...
            if(Resultado.find()==true){
                new Enviar(Admin1.this).execute();
            }else{
    ...
    public void leer(View view) {
        ...
        String url = "http://"+IP+"/android/obtenerPT.php";
    ...
    public void Modifi(View view) {
        if(nuevo.getText().length()==0 || ide.getText().length()==0){
            ...
            if(campo.getSelectedItem().toString().equals("Campo")){
                ...
                new Enviar2(Admin1.this).execute();
            }
        }
    }
    ...
    public void Eliminar(View view) {
        if(id.getText().length()==0){
            Toast.makeText(getApplicationContext(),"Ingrese ID de paciente",Toast.LENGTH_SHORT).show();
        }else{
            new Enviar3(Admin1.this).execute();
        }
    }
        ...
        protected String doInBackground(String... strings) {
            if(Insertar3()){
                contexto.runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                        Toast.makeText(getApplicationContext(),"El paciente ha sido eliminado",Toast.LENGTH_SHORT).show();
                        id.setText("");
                    }
                });
            }else{
    ...
    //Función para mandar los datos necesarios para crear un nuevo paciente
    ...
    //Función para mandar los datos necesarios para modificar un paciente
    public boolean Insertar2(){
        ...
        httpClient = new DefaultHttpClient();
        String IP = getString(R.string.ip);
        httpPost = new HttpPost("http://"+IP+"/android/modificar.php?Id="+ide.getText()+"&Campo="+campo.getSelectedItem());
    ...
    //Función para mandar los datos necesarios para eliminar un paciente
    public boolean Insertar3(){
        ...
        httpClient = new DefaultHttpClient();
        String IP = getString(R.string.ip);
        httpPost = new HttpPost("http://"+IP+"/android/eliminar.php?Id="+id.getText());
        nameValuePairs = new ArrayList<>(4);
    ...
    //Función en caso de que la petición para obtener los pacientes no se completo correctamente
    public void onErrorResponse(VolleyError error) {
        Toast.makeText(getApplicationContext(),"Error al cargar pacientes",Toast.LENGTH_SHORT).show();
    }

    //Funcion en caso de que la peticón para obtener los pacientes se completo correctamente
